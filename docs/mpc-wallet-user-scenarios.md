...
```
密钥分片分配（3个分片）：
├── 分片1：存储在Client Signer或指定成员设备
│   ├── 如果部署了Client Signer：存储在Client Signer服务器
│   ├── 如果未部署Client Signer：存储在指定团队成员设备（如Owner）
│   └── 加密存储在Secure Enclave/TrustZone中
├── 分片2：云端代理1
│   ├── 加密存储在云端TEE环境
│   ├── 多区域备份
│   └── 服务提供商管理
...
- ✅ **团队审批是灵活的**：可以根据策略配置需要多少个团队成员批准
- ✅ **两个层面独立工作**：
  - 技术层面：3个密钥分片协作完成MPC签名
  - 业务层面：多个团队成员批准交易请求
...
sequenceDiagram
    ...
    Note over Member,Blockchain: 业务层面：团队审批流程
    
    Member->>Server: 1. 创建转账请求
    Server->>Server: 2. 评估多签策略<br/>需要2个Manager批准
    Server->>Manager1: 3. 推送审批通知
    Server->>Manager2: 4. 推送审批通知
    Manager1->>Server: 5. 通过APP批准
    Manager2->>Server: 6. 通过APP批准
    
    Note over Server,Blockchain: 技术层面：MPC签名流程（达到审批阈值后启动）
    
    Server->>Signer: 7. 通知使用分片1参与签名
    Server->>Proxy1: 8. 通知使用分片2参与签名
    Server->>Proxy2: 9. 通知使用分片3参与签名
    
    Note over Signer,Proxy2: 10. MPC协议执行（分片不离开各自环境）
    
    Signer->>Signer: 11. 使用分片1计算签名分片
    Proxy1->>Proxy1: 12. 使用分片2计算签名分片
    Proxy2->>Proxy2: 13. 使用分片3计算签名分片
    
    Signer->>Server: 14. 发送签名分片1
    Proxy1->>Server: 15. 发送签名分片2
    Proxy2->>Server: 16. 发送签名分片3
    
    Note over Server: 17. 聚合三个签名分片，生成完整签名
    
    Server->>Server: 18. 聚合签名分片
    Server->>Blockchain: 19. 广播交易
    Blockchain->>Server: 20. 返回交易哈希
    Server->>Member: 21. 通知交易完成
```

**关键理解**：

1. **两个层面独立工作**：
   - **业务层面**：团队审批（需要多少个Manager批准）
   - **技术层面**：MPC签名（需要3个密钥分片参与）

2. **分片持有者**：
   - **分片1**：Client Signer持有（如果部署了）或指定成员设备
   - **分片2**：云端代理1持有
   - **分片3**：云端代理2持有

3. **审批和签名的关系**：
   - 团队成员通过APP批准（业务层面）
   - 达到审批阈值后，启动MPC签名（技术层面）
   - 3个密钥分片协作完成签名（技术层面）

4. **常见误解澄清**：
   - ❌ **误解**：每个团队成员都持有密钥分片
   - ✅ **正确**：密钥分片只有3个，存储在固定位置
   - ✅ **正确**：团队成员通过APP批准，不直接持有分片
   - ✅ **正确**：只有部署了Client Signer的团队，分片1才存储在Client Signer中
...
- 系统自动创建钱包和密钥分片（3个分片）
- **密钥分片分配**（自动完成，用户无需操作）：
  - **分片1**：如果部署了Client Signer，存储在Client Signer服务器；否则存储在云端代理
  - **分片2**：存储在云端代理1（TEE环境）
  - **分片3**：存储在云端代理2（TEE环境）
- 显示钱包地址和二维码
- 发送邀请通知给团队成员
...
- ⚠️ **团队审批是业务层面的**：团队成员通过APP批准，不直接持有密钥分片
- ✅ **两个层面独立**：密钥分片分配和多签策略是两个不同的概念
...
**技术层面（密钥分片）**：
- 团队钱包的密钥分片分配和个人钱包类似，固定为3个分片：
  - **分片1**：如果部署了Client Signer，存储在Client Signer服务器；如果未部署，存储在云端代理
  - **分片2**：存储在云端代理1（TEE环境）
  - **分片3**：存储在云端代理2（TEE环境）
...
- ✅ **正确**：团队成员通过APP批准，这是业务层面的审批，不是技术层面的分片
- ✅ **正确**：只有部署了Client Signer的团队，分片1才存储在Client Signer中
